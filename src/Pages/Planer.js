...
import { getContracts, getUsers } from '../Utils/PlannerService';
...
const Planner = () => {
  ...
  useEffect(() => {
    const fetchContracts = async () => {
      try {
        const response = await getContracts(config);
    ...
    const fetchUsers = async () => {
      try {
        ...
        console.log(response);
        const filteredVendors = response.filter((user) => user.role === 'Vendor');
        setVendors(filteredVendors);
        const filteredProjectContractors = response.filter((user) => user.role === 'Contractor');
